...
class UserController
{
    ...
    {
        $um = new UserModel($data['username'], $data['mail'], $data['bio'], $data['name'], $data['surname'], $data['birthday'], $data['nationality'], $data['elo'], $data['followers'], $data['follow']);
    ...
    public function insertNewUser(array $data)
    {
        ...
        $res = $this->qb->insert("`mail`, `password`, `username`, `name`, `surname`, `birthday`, `nationality`, `elo`")
            ->into('`users`')
            ->value($data['mail'], 's')
